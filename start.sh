...
echo "  3) Docker       — docker compose (dev oder prod)"
echo ""
read -rp "Auswahl [1/2/3]: " mode_choice
...
if [[ "$RUN_MODE" == "docker" ]]; then
  echo ""
  echo "Docker-Modus:"
  echo "  1) Dev   — postgres + backend als Container, frontend lokal (npm run dev)"
  echo "  2) Prod  — alle Services als Container"
  echo ""
  read -rp "Auswahl [1/2]: " docker_choice
  [[ "$docker_choice" == "2" ]] && DOCKER_MODE="prod" || DOCKER_MODE="dev"
fi
...
# ═════════════════════════════════════════════════════════════════════════════
if [[ "$RUN_MODE" == "docker" ]]; then
  # Docker-Modus: eigene Konfiguration in .env.docker
  DOCKER_ENV_FILE="$ROOT_DIR/.env.docker"
  if [[ ! -f "$DOCKER_ENV_FILE" ]]; then
    echo ""
    echo -e "${C_BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${C_RESET}"
    echo -e "${C_BOLD}  Docker-Konfiguration — Secrets${C_RESET}"
    echo -e "${C_BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${C_RESET}"

    {
      echo "# Generated by start.sh — do not commit"
      echo "DB_HOST=postgres"
      echo "PORT_POSTGRES=5432"
      ...
      if [[ "$DOCKER_MODE" == "prod" ]]; then
        read -rp "  Nginx-Port    [80]:   " v; echo "NGINX_PORT=${v:-80}"
      fi
    } > "$DOCKER_ENV_FILE"

    echo ""
    success "Konfiguration gespeichert → .env.docker"
    info    "Zurücksetzen mit: rm .env.docker && ./start.sh"
    echo ""
  fi
...
# ═════════════════════════════════════════════════════════════════════════════
# KONFIGURATION LADEN (nicht für Docker — dort lädt compose .env.docker)
# ═════════════════════════════════════════════════════════════════════════════
if [[ "$RUN_MODE" != "docker" ]]; then
  set -a; source "$CONFIG_FILE"; set +a
fi

# H2-Defaults (nur für lokalen Dev-Modus)
if [[ "$RUN_MODE" == "dev" && "$DB_MODE" == "h2" ]]; then
...
if [[ "$RUN_MODE" == "docker" ]]; then
  if [[ "$DOCKER_MODE" == "prod" ]]; then
    ...
    COMPOSE_LABEL="Prod (alle Services)"
  else
    COMPOSE_FILE="docker-compose.dev.yml"
    COMPOSE_LABEL="Dev (postgres + backend)"
  fi

  echo -e "${C_BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${C_RESET}"
  echo -e "${C_BOLD}  HackSMC — Docker-Modus${C_RESET}"
  echo -e "${C_BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${C_RESET}"
  info "Compose-Datei → $COMPOSE_FILE ($COMPOSE_LABEL)"
  echo ""

  if [[ "$DOCKER_MODE" == "dev" ]]; then
    info "Frontend läuft lokal → cd frontend && npm run dev"
    echo ""
  fi

  docker compose -f "$ROOT_DIR/$COMPOSE_FILE" --env-file "$ROOT_DIR/.env.docker" up --build
fi
...
if [[ "$RUN_MODE" == "dev" ]]; then
  ...
  (
    cd "$ROOT_DIR/backend"
    if [[ "$DB_MODE" == "h2" ]]; then
      mvn spring-boot:run -q -Dspring-boot.run.profiles=h2,dev -Dspring-boot.run.jvmArguments="-Djava.net.preferIPv4Stack=true -XX:TieredStopAtLevel=1 -Xmx768m"
    else
      mvn spring-boot:run -q -Dspring-boot.run.profiles=dev -Dspring-boot.run.jvmArguments="-Djava.net.preferIPv4Stack=true -XX:TieredStopAtLevel=1 -Xmx768m"
    fi
  ) &
  BACKEND_PID=$!
