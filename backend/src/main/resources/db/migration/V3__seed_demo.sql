...
INSERT INTO nat_rules (user_id, host_id, protocol, port, description, pf_sense_rule_id, status, created_at) VALUES
  (2, 1, 'TCP', 25565, 'Minecraft Java Edition',  'pf-rule-001', 'ACTIVE',  NOW() - INTERVAL '3' DAY),
  (2, 2, 'UDP', 9987,  'TeamSpeak voice',          'pf-rule-002', 'ACTIVE',  NOW() - INTERVAL '1' DAY);

-- ── NAT rules for lisa ────────────────────────────────────────────────────────
INSERT INTO nat_rules (user_id, host_id, protocol, port, description, pf_sense_rule_id, status, created_at) VALUES
  (3, 3, 'TCP', 80,   'HTTP frontend',             'pf-rule-003', 'ACTIVE',  NOW() - INTERVAL '5' DAY),
  (3, 3, 'TCP', 443,  'HTTPS frontend',            'pf-rule-004', 'ACTIVE',  NOW() - INTERVAL '5' DAY),
  (3, 3, 'TCP', 8080, 'Dev proxy',                 'pf-rule-005', 'ACTIVE',  NOW() - INTERVAL '2' DAY),
  (3, 4, 'UDP', 51820,'WireGuard',                 'pf-rule-006', 'ACTIVE',  NOW() - INTERVAL '7' DAY);

-- ── NAT rules for felix ───────────────────────────────────────────────────────
INSERT INTO nat_rules (user_id, host_id, protocol, port, description, pf_sense_rule_id, status, created_at) VALUES
  (4, 5, 'TCP', 3000, 'React dev server',          'pf-rule-007', 'ACTIVE',  NOW() - INTERVAL '1' DAY),
  (4, 5, 'TCP', 8000, 'Django backend',            'pf-rule-008', 'ACTIVE',  NOW() - INTERVAL '2' DAY),
  (4, 5, 'UDP', 5000, 'Game server (test)',        NULL,          'DELETED', NOW() - INTERVAL '4' DAY);
